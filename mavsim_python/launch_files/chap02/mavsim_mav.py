...
while sim_time < end_time:
    ...
    if motions_time < time_per_motion:
        ...
        Va = 10
        state.theta -= 0.1*(time_per_motion*2 - motions_time)**2*SIM.ts_simulation
        Rbi = euler_to_rotation(state.phi, state.theta, state.psi)
        Vai = Rbi.T @ np.array([[Va], [0], [0]])
        state.north += Vai[0, 0]*SIM.ts_simulation
        state.east += Vai[1, 0]*SIM.ts_simulation
        state.altitude += Vai[2, 0]*SIM.ts_simulation
        ...
        state.phi += 0.5*SIM.ts_simulation
        Va = 10
        Rbi = euler_to_rotation(state.phi, state.theta, state.psi)
        Vai = Rbi.T @ np.array([[Va], [0], [0]])
        state.north += Vai[0, 0]*SIM.ts_simulation
